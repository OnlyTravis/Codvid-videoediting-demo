...
class APIManager:
    ...
    def group_small_chunks(cls, messages_lists: list[list[BaseMessage]], split_settings: VideoSplitSettings) -> list[Chunk]:
        ...
        for i in range(len(messages_lists)):
            ...
            Logger.log_file(f'group_chunk_raw_output_{i+1}.txt', res)
            res = ai_msg.tool_calls[0]['args']
            if (previous_chunk_data == None):
                ...
                chunks.append(ResponceParser.parse_chunk_output(res['output'][0], split_settings, last_base_index))
            ...
            base_index += len(res['output'])-1 # -1 for padding

            # 4. Save last for next (if any)
            if (i+1 == len(messages_lists)):
                chunks.append(ResponceParser.parse_chunk_output(res['output'][-1], split_settings, base_index+len(res['output'])-1))
                break
    ...
    @classmethod
    def generate_video_script(cls, chunks: list[Chunk], prompt: str) -> str:
        # 1. Create Message
